...
if [ $(id -u) != "0" ];
then
	echo -e "|   Error: You need to be root to install the Allstats agent\n|"
...
if [ -f /etc/allstats/allstats-agent.sh ]
then
	# Remove agent dir
	rm -Rf /etc/allstats

	# Remove cron entry and user
	if id -u allstats >/dev/null 2>&1
	then
		(crontab -u allstats -l | grep -v "/etc/allstats/allstats-agent.sh") | crontab -u allstats - && userdel allstats
	else
		(crontab -u root -l | grep -v "/etc/allstats/allstats-agent.sh") | crontab -u root -
	fi
fi

# Create agent dir
mkdir -p /etc/allstats

# Download agent
echo -e "|   Downloading allstats-agent.sh to /etc/allstats\n|\n|   + $(wget -nv -o /dev/stdout -O /etc/allstats/allstats-agent.sh --no-check-certificate http://allstats.in/static/scripts/allstats-agent.sh)"

if [ -f /etc/allstats/allstats-agent.sh ]
then
	# Create auth file
	echo "$1" > /etc/allstats/allstats-auth.log

	# Create user
	useradd allstats -r -d /etc/allstats -s /bin/false

	# Modify user permissions
	chown -R allstats:allstats /etc/allstats && chmod -R 700 /etc/allstats

	# Modify ping permissions
	chmod +s `type -p ping`

	# Configure cron
	crontab -u allstats -l 2>/dev/null | { cat; echo "*/3 * * * * bash /etc/allstats/allstats-agent.sh > /etc/allstats/allstats-cron.log 2>&1"; } | crontab -u allstats -

	# Show success
	echo -e "|\n|   Success: The Allstats agent has been installed\n|"
	...
	# Show error
	echo -e "|\n|   Error: The Allstats agent could not be installed\n|"
fi