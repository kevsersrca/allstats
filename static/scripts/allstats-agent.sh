...
if [ -f /etc/allstats/allstats-auth.log ]
then
	auth=($(cat /etc/allstats/allstats-auth.log))
else
...
if [ -e /etc/allstats/allstats-data.log ]
then
	data=($(cat /etc/allstats/allstats-data.log))
...
# System load cache
echo "$time $cpu $io $idle $rx $tx" > /etc/allstats/allstats-data.log
...
then
	timeout -s SIGKILL 30 wget -q -o /dev/null -O /etc/allstats/allstats-agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://allstats.in/api/agent.json"
else
	wget -q -o /dev/null -O /etc/allstats/allstats-agent.log -T 25 --post-data "$data_post" --no-check-certificate "http://allstats.in/api/agent.json"
	wget_pid=$!
	wget_counter=0
	wget_timeout=30
